...
macro_rules! impl_lua_conversion_dynamic {
  ($struct:ident) => {
    impl $crate::mlua::IntoLua for $struct {
      fn into_lua(self, lua: &$crate::mlua::Lua) -> Result<$crate::mlua::Value, $crate::mlua::Error> {
        $crate::to_lua(lua, self)
      }
    }

    impl $crate::mlua::FromLua for $struct {
      fn from_lua(value: $crate::mlua::Value, _lua: &$crate::mlua::Lua) -> Result<Self, $crate::mlua::Error> {
  ...
  Ok(match value {
    ...
    DynValue::Object(object) => {
      ...
      for (key, value) in object.into_iter() {
        table.set(dynamic_to_lua_value(lua, key)?, dynamic_to_lua_value(lua, value)?)?;
      }
      LuaValue::Table(table)
    }
  })
...
fn lua_value_to_dynamic_impl(value: LuaValue, visited: &mut HashSet<usize>) -> mlua::Result<DynValue> {
          ...
          match to_dynamic.call(LuaValue::UserData(ud.clone())) {
            ...
            Err(err) => {
              return Err(mlua::Error::FromLuaConversionError {
                from: "userdata",
                to: "wezterm_dynamic::Value".to_string(),
                message: Some(format!("error calling __wezterm_to_dynamic: {err:#}")),
              })
            }
          }
        }
          ...
          match &key {
            ...
            _ => {
              ...
              return Err(mlua::Error::FromLuaConversionError {
                ...
                message: Some(format!("Unexpected key {key:?} for array style table")),
              });
            }
          }
        }
...
impl ValuePrinterHelper {
  fn has_cycle(&self, value: &mlua::Value) -> bool {
    self.visited.borrow().contains(&(value.to_pointer() as usize))
  }
}
...
impl std::fmt::Debug for ValuePrinterHelper {
  fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::result::Result<(), std::fmt::Error> {
    match &self.value {
      ...
      LuaValue::UserData(_) if self.is_cycle => fmt.write_fmt(format_args!("userdata: {:?}", self.value.to_pointer())),
