...
use wezterm_dynamic::{
  Error as DynError, FromDynamic, FromDynamicOptions, ToDynamic, UnknownFieldAction, Value as DynValue,
};
  ...
  fn add_methods<'lua, M: UserDataMethods<Self>>(methods: &mut M) {
    ...
    methods.add_meta_method(MetaMethod::Index, |lua, _myself, field: String| {
      ...
      match T::from_dynamic(
        ...
        err @ Err(DynError::InvalidVariantForType { .. }) => Err(mlua::Error::external(err.unwrap_err().to_string())),
        _ => {
